...
# Fall 2019
# CS232 Project 8
# assembler.py : read in files without function arguments and convert
	...
	for line in tokens:
		if line[0][-1] == ":":
			...
			if line[0][:-1] in symbols:
				print("error : duplicate symbol detected : \"" + line[0][:-1] + "\"")
				exit()
    ...
    for i,line in enumerate(tokens):
        ...
        try:
            if instr == "load":
                ...
                print("error : instruction not defined : \"" + line[0] + "\"")
                exit()
        # KeyError : access a key that is not in the dictionary
        except KeyError as e:
            print("error : key not found : " + str(e))
    return code
...
def main( argv ):
    ...
    symbols = pass1(tokens)
    instructions = pass2(tokens, symbols)
